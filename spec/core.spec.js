...
describe('#parser', function () {
    ...
    it('should throw error when an object passed without path', function () {
        expect(()=>def.parser({validators: []})).toThrow();
    });

    it('should throw error when an object passed without validators', function () {
        expect(()=>def.parser({path: ''})).toThrow();
    });


    it('should correctly parse object passed', function () {
        expect(def.parser({path: '', validators: []}))
            .toEqual({path: [], validators: [], conditions: []});
    });
    it('should take conditions from the schema passed', function () {
        expect(def.parser({path: '', validators: [], conditions: ['a']}))
            .toEqual({path: [], validators: [], conditions: ['a']});
    });

    it('should take conditions from the string expression passed', function () {
        expect(def.parser('a#b?c?d'))
            .toEqual({path: ['a'], validators: ['b'], conditions: ['c', 'd']});
    });

});

describe('#validatorProvider', function () {
